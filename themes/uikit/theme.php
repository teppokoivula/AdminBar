...
$this->addHookAfter('AdminBar::getItems', function(HookEvent $event) {
    ...
    if ($has_icons) {
        ...
        $icons = array_filter([
            'browse' => $has_icons == 10 || $has_icons == 30 ? [
                'left',
            ...
            'edit' => $has_icons == 10 || $has_icons == 30 ? [
                'left',
                '<path d="M491 1536l91-91-235-235-91 91v107h128v128h107zm523-928q0-22-22-22-10 0-17 7l-542 542q-7 7-7 17 0 22 22 22 10 0 17-7l542-542q7-7 7-17zm-54-192l416 416-832 832h-416v-416zm683 96q0 53-37 90l-166 166-416-416 166-165q36-38 90-38 53 0 91 38l235 234q37 39 37 91z" fill="#fff"/>',
            ] : null,
            'new' => $has_icons == 10 || $has_icons == 30 ? [
                'left',
            ...
            'profile' => $has_icons >= 20 ? [
                'right',
                '<path d="M1536 1399q0 109-62.5 187t-150.5 78h-854q-88 0-150.5-78t-62.5-187q0-85 8.5-160.5t31.5-152 58.5-131 94-89 134.5-34.5q131 128 313 128t313-128q76 0 134.5 34.5t94 89 58.5 131 31.5 152 8.5 160.5zm-256-887q0 159-112.5 271.5t-271.5 112.5-271.5-112.5-112.5-271.5 112.5-271.5 271.5-112.5 271.5 112.5 112.5 271.5z" fill="#fff"/>',
            ] : null,
            'logout' => $has_icons >= 20 ? [
                'right',
        ...
        foreach ($icons as $item => $icon) {
            list($column, $path) = $icon;
            if (empty($items[$column][$item]['text'])) continue;
            if ($item == 'edit' && empty($items[$column][$item]['link'])) {
            ...
            $items[$column][$item]['text'] = sprintf($svg, $path) . $items[$column][$item]['text'];
        }
    }
